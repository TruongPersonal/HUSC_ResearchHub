...
public interface TopicMemberRepository extends JpaRepository<TopicMember, Integer> {
  ...
  @Query(
      "SELECT COUNT(tm) FROM TopicMember tm JOIN tm.topic t "
          + "WHERE tm.user.id = :userId "
          + "AND tm.role = :role "
          + "AND tm.status = 'APPROVED' "
          + "AND t.academicYear.id = :academicYearId")
  long countByUserIdAndRoleAndAcademicYear(
  ...
  @Query(
      "SELECT COUNT(tm) FROM TopicMember tm JOIN tm.topic t "
          ...
          + "AND tm.status = 'APPROVED' "
          + "AND t.academicYear.id = :academicYearId")
  long countByUserIdAndAcademicYear(
      @Param("userId") Integer userId, @Param("academicYearId") Integer academicYearId);
}
