...
public class YearSessionController {
  ...
  /**
   * Lấy danh sách đợt đăng ký theo điều kiện lọc (từ khóa, khoa, trạng thái)
   */
  @GetMapping
  public ResponseEntity<PageResponse<YearSessionResponse>> getAll(
    ...
    Page<YearSessionResponse> result =
        yearSessionService.getAll(keyword, page, size, departmentId, status);
   ...
   */
  @PostMapping
  public ResponseEntity<YearSessionResponse> create(
      @Valid @RequestBody YearSessionRequest request) {
   ...
   */
  @PutMapping("/{id}")
  public ResponseEntity<YearSessionResponse> update(
      @PathVariable Integer id, @Valid @RequestBody YearSessionRequest request) {
  ...
  /**
   * Lấy danh sách các năm học đang hoạt động để mở đợt đăng ký
   */
  @GetMapping("/available-years")
  public ResponseEntity<List<AcademicYear>> getAvailableYears() {
    return ResponseEntity.ok(
        academicYearRepository.findByIsActiveTrueAndStatus(AcademicYearStatus.START));
  }
}
