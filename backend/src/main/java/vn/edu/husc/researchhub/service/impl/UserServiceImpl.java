...
public class UserServiceImpl implements UserService {
  ...
  public Page<UserResponse> getAll(
      String keyword, Role role, Integer departmentId, int page, int size) {
  ...
  public UserResponse create(UserRequest request) {
    ...
    // Restrict creation to only STUDENT and TEACHER
    if (request.getRole() != Role.STUDENT && request.getRole() != Role.TEACHER) {
      throw new RuntimeException("Chỉ được phép thêm Sinh viên hoặc Giảng viên");
    }

    // Require Department for both STUDENT and TEACHER
    if ((request.getRole() == Role.STUDENT || request.getRole() == Role.TEACHER)
        && request.getDepartmentId() == null) {
      throw new RuntimeException("Sinh viên và Giảng viên bắt buộc phải thuộc một khoa");
    }
  ...
  public UserResponse update(Integer id, UserRequest request) {
    ...
    // Restrict role changes generally to Student/Teacher if role is being updated
    if (request.getRole() != null) {
      if (request.getRole() != Role.STUDENT && request.getRole() != Role.TEACHER) {
          throw new RuntimeException("Chỉ được phép cập nhật vai trò thành Sinh viên hoặc Giảng viên");
      }
    }
              ...
              .orElseThrow(
                  () ->
                      new RuntimeException(
                          "Không tìm thấy khoa với ID: " + request.getDepartmentId()));
      user.setDepartment(department);
    } else if (user.getDepartment() == null && (user.getRole() == Role.STUDENT || user.getRole() == Role.TEACHER)) {
      throw new RuntimeException("Vui lòng chọn Khoa cho người dùng này");
    }
  ...
  public UserResponse updateProfile(
    ...
    if (request.getFullName() != null)
      user.setFullName(request.getFullName().isEmpty() ? null : request.getFullName());
    ...
    if (request.getClassName() != null)
      user.setClassName(request.getClassName().isEmpty() ? null : request.getClassName());
    if (request.getAcademicDegree() != null)
      user.setAcademicDegree(
          request.getAcademicDegree().isEmpty() ? null : request.getAcademicDegree());
  ...
  public String updateAvatar(
      String username, org.springframework.web.multipart.MultipartFile file) {
    User user =
        userRepository
            .findByUsername(username)
            .orElseThrow(() -> new RuntimeException("Không tìm thấy người dùng: " + username));
  ...
  public void forgotPassword(String username) {
    ...
    // Only allow STUDENT and TEACHER
    if (user.getRole() != Role.STUDENT && user.getRole() != Role.TEACHER) {
      throw new RuntimeException("Tính năng này chỉ dành cho Sinh viên và Giảng viên");
    }
...
// ...
  ...
  @Override
  public java.util.List<vn.edu.husc.researchhub.dto.response.ChatPartnerResponse> findChatPartners(String username, String query) {
    User currentUser = userRepository.findByUsername(username)
        .orElseThrow(() -> new RuntimeException("User not found"));

    java.util.List<User> partners = new java.util.ArrayList<>();
    String safeQuery = (query == null) ? "" : query.toLowerCase().trim();

    // Logic based on Role
    if (currentUser.getRole() == Role.STUDENT) {
      // Students can chat with: Students and Lecturers in SAME Department
      if (currentUser.getDepartment() != null) {
        partners.addAll(userRepository.searchChatPartners(
            currentUser.getDepartment().getId(),
            java.util.List.of(Role.STUDENT, Role.TEACHER),
            safeQuery,
            currentUser.getId() // Exclude self
        ));
      }
    } else if (currentUser.getRole() == Role.TEACHER) {
      // Lecturers can chat with: Students and Assistants in SAME Department
      if (currentUser.getDepartment() != null) {
        partners.addAll(userRepository.searchChatPartners(
            currentUser.getDepartment().getId(),
            java.util.List.of(Role.STUDENT, Role.ASSISTANT),
            safeQuery,
            currentUser.getId()
        ));
      }
    } else if (currentUser.getRole() == Role.ASSISTANT) {
      // Assistants can chat with:
      // 1. Lecturers in SAME Department
      if (currentUser.getDepartment() != null) {
        partners.addAll(userRepository.searchChatPartners(
            currentUser.getDepartment().getId(),
            java.util.List.of(Role.TEACHER),
            safeQuery,
            currentUser.getId()
        ));
      }
      // 2. Admins (Global)
      partners.addAll(userRepository.searchGlobalChatPartners(
          java.util.List.of(Role.ADMIN),
          safeQuery,
          currentUser.getId()
      ));
    } else if (currentUser.getRole() == Role.ADMIN) {
      // Admins can chat with: Assistants (Global)
      partners.addAll(userRepository.searchGlobalChatPartners(
          java.util.List.of(Role.ASSISTANT),
          safeQuery,
          currentUser.getId()
      ));
    }
    ...
    // Convert to DTO
    // Note: We are ignoring "last message" and "unread count" for now in this search method
    // In a real app, this would be computed or fetched via a separate efficient query
    return partners.stream().map(u -> ChatPartnerResponse.builder()
        .id(u.getId())
