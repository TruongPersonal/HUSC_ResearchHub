...
public class AcademicYearServiceImpl implements AcademicYearService {
    ...
    @Override
    public Page<AcademicYearResponse> getAllAcademicYears(String keyword, Boolean isActive, int page, int size) {
        Pageable pageable = PageRequest.of(page, size, Sort.by("year").descending());
        Page<AcademicYear> academicYearPage = academicYearRepository.search(keyword, isActive, pageable);
    ...
    public AcademicYearResponse updateAcademicYear(Integer id, AcademicYearRequest request) {
        ...
        if (!academicYear.getYear().equals(request.getYear()) && academicYearRepository.existsByYear(request.getYear())) {
            throw new RuntimeException("Năm học đã tồn tại: " + request.getYear());
        }

        if (request.getStatus() != null && request.getStatus() == vn.edu.husc.researchhub.model.enums.AcademicYearStatus.END) {
             // Validate all Year Sessions are COMPLETED
            java.util.List<vn.edu.husc.researchhub.model.YearSession> sessions = yearSessionRepository.findByAcademicYearId(id);
            
            boolean hasNonCompleted = sessions.stream()
                    .anyMatch(s -> s.getStatus() != vn.edu.husc.researchhub.model.enums.YearSessionStatus.COMPLETED);
            
            if (hasNonCompleted) {
                throw new RuntimeException("Không thể kết thúc năm học vì chưa hoàn thành tất cả các phiên làm việc của các khoa");
            }
        }
