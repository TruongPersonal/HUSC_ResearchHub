...
public class TopicServiceImpl implements TopicService {
    ...
    @Override
    public PageResponse<TopicResponse> getAllTopics(String keyword, TopicStatus status, Integer departmentId, Integer academicYearId, int page, int size) {
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdAt").descending());
        
        Page<Topic> topicPage = topicRepository.search(departmentId, academicYearId, keyword, status, pageable);
    ...
    public void updateTopicStatus(Integer id, TopicStatus status, String feedback) {
        ...
        if ((status == TopicStatus.NEEDS_UPDATE || status == TopicStatus.REJECTED) && feedback != null && !feedback.isEmpty()) {
            String currentNote = topic.getNote() != null ? topic.getNote() : "";
            String prefix = status == TopicStatus.NEEDS_UPDATE ? "[Yêu cầu bổ sung]: " : "[Lý do từ chối]: ";
        ...
        if (status == TopicStatus.APPROVED) {
            if (!approvedTopicRepository.existsByTopicId(id)) {
                vn.edu.husc.researchhub.model.ApprovedTopic approvedTopic = new vn.edu.husc.researchhub.model.ApprovedTopic();
                approvedTopic.setTopic(topic);
                approvedTopic.setStatus(vn.edu.husc.researchhub.model.enums.ApprovedTopicStatus.IN_PROGRESS);
    ...
    @Override
    @Transactional
    public void updateTopic(Integer id, vn.edu.husc.researchhub.dto.request.UpdateTopicRequest request) {
        Topic topic = topicRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy đề tài"));
        ...
        java.util.Optional<vn.edu.husc.researchhub.model.ApprovedTopic> approvedTopicOpt = approvedTopicRepository.findByTopicId(id);
    ...
    public void assignLeader(Integer topicId, Integer studentId) {
        ...
        if (studentId != null) {
            ...
            TopicMember targetMember = topicMemberRepository.findByTopicIdAndUserId(topicId, studentId)
                    .orElseThrow(() -> new RuntimeException("Sinh viên này chưa là thành viên của đề tài"));
    ...
    public void approveMember(Integer topicId, Integer studentId) {
        TopicMember member = topicMemberRepository.findByTopicIdAndUserId(topicId, studentId)
    ...
    public void proposeTopic(vn.edu.husc.researchhub.dto.request.ProposeTopicRequest request) {
        ...
        vn.edu.husc.researchhub.model.YearSession session = yearSessionRepository.findByAcademicYearIdAndDepartmentId(academicYearId, departmentId)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy phiên làm việc cho khoa và năm học này"));
        
        if (session.getStatus() != vn.edu.husc.researchhub.model.enums.YearSessionStatus.ON_REGISTRATION) {
        ...
        topic.setBudget(request.getBudget() != null ? java.math.BigDecimal.valueOf(request.getBudget()) : java.math.BigDecimal.ZERO);
        topic.setNote(request.getNote());
    ...
    public void registerTopic(Integer topicId) {
        ...
        boolean isMember = topicMemberRepository.findByTopicIdAndUserId(topicId, student.getId()).isPresent();
        if (isMember) {
    ...
    private TopicResponse mapToResponse(Topic topic) {
        ...
        TopicMember advisor = members.stream()
                .filter(m -> m.getRole() == TopicMemberRole.ADVISOR)
                .findFirst().orElse(null);
                
        TopicMember leader = members.stream()
                .filter(m -> m.getRole() == TopicMemberRole.LEADER && m.getStatus() == MemberStatus.APPROVED)
                .findFirst().orElse(null);
                
        List<TopicResponse.MemberResponse> pendingMembers = members.stream()
                .filter(m -> m.getStatus() == MemberStatus.PENDING)
                .map(m -> new TopicResponse.MemberResponse(
                        m.getUser().getId(),
        ...
        List<TopicResponse.MemberResponse> approvedMembers = members.stream()
                .filter(m -> m.getStatus() == MemberStatus.APPROVED && m.getUser().getRole() == Role.STUDENT)
                .map(m -> new TopicResponse.MemberResponse(
                        m.getUser().getId(),
                        m.getUser().getFullName(),
                        m.getUser().getEmail(),
        ...
        List<TopicResponse.MemberResponse> rejectedMembers = members.stream()
        ...
        return TopicResponse.builder()
                .id(topic.getId())
                .code(approvedTopicRepository.findByTopicId(topic.getId())
                        .map(vn.edu.husc.researchhub.model.ApprovedTopic::getCode)
                        .orElse(null))
                .prize(approvedTopicRepository.findByTopicId(topic.getId())
                ...
                .note(topic.getNote())
                .researchField(approvedTopicRepository.findByTopicId(topic.getId())
                        .map(vn.edu.husc.researchhub.model.ApprovedTopic::getFieldResearch)
                        .orElse(null))
                .researchType(approvedTopicRepository.findByTopicId(topic.getId())
                        .map(vn.edu.husc.researchhub.model.ApprovedTopic::getTypeResearch)
                        .orElse(null))
                .status(topic.getStatus())
                .approvedStatus(approvedTopicRepository.findByTopicId(topic.getId())
                        .map(vn.edu.husc.researchhub.model.ApprovedTopic::getStatus)
                        .orElse(null))
                .createdAt(topic.getCreatedAt())
                .sessionStatus(yearSessionRepository.findByAcademicYearIdAndDepartmentId(
                        topic.getAcademicYear().getId(),
                ...
                .advisors(members.stream()
                        .filter(m -> m.getRole() == TopicMemberRole.ADVISOR && m.getStatus() == MemberStatus.APPROVED)
                        .map(m -> new TopicResponse.MemberResponse(
                                m.getUser().getId(),
