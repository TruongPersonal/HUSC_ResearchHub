...
public interface AcademicYearRepository extends JpaRepository<AcademicYear, Integer> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT a FROM AcademicYear a WHERE " +
            "(:keyword IS NULL OR :keyword = '' OR CAST(a.year AS string) LIKE CONCAT('%', :keyword, '%')) AND " +
    ...
    java.util.List<AcademicYear> findByIsActiveTrueAndStatus(vn.edu.husc.researchhub.model.enums.AcademicYearStatus status);
}
