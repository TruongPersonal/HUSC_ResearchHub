...
public class DashboardServiceImpl implements DashboardService {
    ...
    public DashboardStatsResponse getStats(Integer academicYearId) {
        ...
        if (academicYearId != null) {
            currentYear = academicYearRepository.findById(academicYearId)
             ...
             // Let's assume if academicYearId is null, it means "All years" context, but usually the frontend sends a year.
             // Let's look at the frontend hook `useAdminStats`. It sends `selectedYear.id`.
             // So if selectedYear is null, it might return.
        }
        
        // Actually, let's just use the same logic as getAssistantStats for consistency if ID is provided.
        if (academicYearId != null) {
             currentYear = academicYearRepository.findById(academicYearId)
                .map(ay -> ay.getYear().toString())
                .orElse("Không xác định");
    ...
    @Override
    public vn.edu.husc.researchhub.dto.response.AssistantDashboardStatsResponse getAssistantStats(Integer departmentId, Integer academicYearId) {
        if (departmentId == null || academicYearId == null) {
            return new vn.edu.husc.researchhub.dto.response.AssistantDashboardStatsResponse(0, 0, "Chưa chọn năm học", "Không xác định");
        }

        long registrationCount = topicRepository.countByDepartmentIdAndAcademicYearId(departmentId, academicYearId);
        long approvedTopicCount = approvedTopicRepository.countByTopicDepartmentIdAndTopicAcademicYearId(departmentId, academicYearId);
        
        var academicYear = academicYearRepository.findById(academicYearId).orElse(null);
        String currentYear = academicYear != null ? academicYear.getYear().toString() : "Không xác định";
        
        String status = "Không xác định";
        if (academicYear != null && academicYear.getStatus() != null) {
            status = switch (academicYear.getStatus()) {
                case START -> "Đang diễn ra";
                case END -> "Đã kết thúc";
            };
        }

        return new vn.edu.husc.researchhub.dto.response.AssistantDashboardStatsResponse(registrationCount, approvedTopicCount, currentYear, status);
    }
}
