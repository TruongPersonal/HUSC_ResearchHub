...
public interface ApprovedTopicRepository extends JpaRepository<ApprovedTopic, Integer> {
    ...
    java.util.List<ApprovedTopic> findByTopicDepartmentIdAndTopicAcademicYearId(Integer departmentId, Integer academicYearId);

    @Query("SELECT at FROM ApprovedTopic at JOIN at.topic t WHERE " +
           "(:departmentId IS NULL OR t.department.id = :departmentId) AND " +
           "(:academicYearId IS NULL OR t.academicYear.id = :academicYearId) AND " +
           "(:keyword IS NULL OR :keyword = '' OR t.name LIKE %:keyword% OR at.code LIKE %:keyword%) AND " +
