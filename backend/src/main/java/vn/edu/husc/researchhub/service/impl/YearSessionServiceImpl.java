...
public class YearSessionServiceImpl implements YearSessionService {
    ...
    @Override
    public Page<YearSessionResponse> getAll(String keyword, int page, int size, Integer departmentId, YearSessionStatus status) {
        Pageable pageable = PageRequest.of(page, size, Sort.by("year").descending());
        Page<YearSession> sessions = yearSessionRepository.search(keyword, departmentId, status, pageable);
    ...
    public YearSessionResponse create(YearSessionRequest request) {
        ...
        if (yearSessionRepository.existsByAcademicYearIdAndDepartmentId(request.getAcademicYearId(), request.getDepartmentId())) {
        ...
        session.setStatus(request.getStatus() != null ? request.getStatus() : YearSessionStatus.ON_REGISTRATION);
    ...
    public YearSessionResponse update(Integer id, YearSessionRequest request) {
        YearSession session = yearSessionRepository.findById(id)
        ...
        if (request.getStatus() != null) {
            ...
            if (request.getStatus() == YearSessionStatus.COMPLETED) {
                ...
                java.util.List<vn.edu.husc.researchhub.model.ApprovedTopic> approvedTopics = approvedTopicRepository
                        .findByTopicDepartmentIdAndTopicAcademicYearId(session.getDepartment().getId(), session.getAcademicYear().getId());
                
                boolean hasInvalidStatus = approvedTopics.stream()
                        .anyMatch(at -> at.getStatus() == vn.edu.husc.researchhub.model.enums.ApprovedTopicStatus.IN_PROGRESS 
                                     || at.getStatus() == vn.edu.husc.researchhub.model.enums.ApprovedTopicStatus.NOT_COMPLETED);
                
                if (hasInvalidStatus) {
                    throw new RuntimeException("Không thể hoàn thành phiên năm học vì còn đề tài chưa hoàn tất hoặc không hoàn thành.");
                }
                ...
                java.util.List<vn.edu.husc.researchhub.model.Topic> topics = topicRepository
                        .findByDepartmentIdAndAcademicYearId(session.getDepartment().getId(), session.getAcademicYear().getId());
                
                boolean hasPending = topics.stream()
                        .anyMatch(t -> t.getStatus() == vn.edu.husc.researchhub.model.enums.TopicStatus.PENDING 
                                    || t.getStatus() == vn.edu.husc.researchhub.model.enums.TopicStatus.NEEDS_UPDATE);
                
                if (hasPending) {
                    throw new RuntimeException("Không thể chuyển sang trạng thái Đang thực hiện vì còn hồ sơ chưa được Duyệt hoặc Từ chối.");
                }
            }
