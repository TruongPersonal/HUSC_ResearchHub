...
public class ApprovedTopicServiceImpl implements ApprovedTopicService {
    ...
    @Override
    public PageResponse<ApprovedTopicResponse> getAllApprovedTopics(Integer departmentId, Integer academicYearId, String keyword, vn.edu.husc.researchhub.model.enums.ApprovedTopicStatus status, int page, int size) {
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdAt").descending());
        Page<ApprovedTopic> approvedTopicPage = approvedTopicRepository.search(departmentId, academicYearId, keyword, status, pageable);

        List<ApprovedTopicResponse> content = approvedTopicPage.getContent().stream()
    ...
    public ApprovedTopicResponse updateApprovedTopic(Integer id, vn.edu.husc.researchhub.dto.request.UpdateApprovedTopicRequest request) {
        ApprovedTopic approvedTopic = approvedTopicRepository.findById(id)
        ...
        if (request.getFieldResearch() != null) approvedTopic.setFieldResearch(request.getFieldResearch());
    ...
    public List<ApprovedTopicDocumentResponse> getDocumentsByTopicId(Integer topicId) {
        ApprovedTopic approvedTopic = approvedTopicRepository.findByTopicId(topicId)
                .orElseThrow(() -> new ResourceNotFoundException("Đề tài chưa được duyệt hoặc không tồn tại"));
    ...
    @Override
    @org.springframework.transaction.annotation.Transactional
    public ApprovedTopicDocumentResponse uploadDocument(Integer topicId, org.springframework.web.multipart.MultipartFile file, vn.edu.husc.researchhub.model.enums.DocumentType type, String summary) {
        // 1. Find Approved Topic by Topic ID
        ApprovedTopic approvedTopic = approvedTopicRepository.findByTopicId(topicId)
                .orElseThrow(() -> new ResourceNotFoundException("Đề tài chưa được duyệt hoặc không tồn tại"));

        // 2. Check if document exists
        ApprovedTopicDocument document = approvedTopicDocumentRepository.findByApprovedTopicId(approvedTopic.getId()).stream()
    ...
    public void deleteDocument(Integer documentId) {
        ApprovedTopicDocument document = approvedTopicDocumentRepository.findById(documentId)
