...
export function AcademicYearProvider({ children }: { children: React.ReactNode }) {
    ...
    const fetchYears = async () => {
        try {
            ...
            if (!yearToSelect) {
                ...
                if (savedYearId) {
                    yearToSelect = activeYears.find(y => y.id.toString() === savedYearId)
                }
            }
            ...
            if (yearToSelect) {
                ...
                localStorage.setItem("selectedAcademicYearId", yearToSelect.id.toString())
            } else {
    ...
    useEffect(() => {
        ...
        // Listen for updates from admin page (when years are added/edited)
        const handleUpdate = () => {
            fetchYears()
        }
        window.addEventListener("academic-year-updated", handleUpdate)
        return () => window.removeEventListener("academic-year-updated", handleUpdate)
    }, [])
    ...
    return (
        <AcademicYearContext.Provider value={{ years, selectedYear, setSelectedYear, loading, refreshYears: fetchYears }}>
            {children}
