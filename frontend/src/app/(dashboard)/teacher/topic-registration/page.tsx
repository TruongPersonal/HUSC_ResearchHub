...
import { AlertCircle, Loader2, Send, ArrowLeft, CheckCircle2, XCircle, RefreshCw, AlertTriangle, FileSignature } from "lucide-react";
        ...
        const initData = async () => {
            ...
            try {
                ...
                if (user.departmentId) {
                    ...
                    const sessionsResponse = await yearSessionService.getAll(undefined, 0, 100, user.departmentId);
                    const currentSession = sessionsResponse.content.find(s =>
                        s.academicYearId === selectedYear.id && s.departmentId === user.departmentId
                    );
                ...
                const rejected = myTopics.filter(t => t.status === "REJECTED" || t.status === "CANCELLED");
    ...
    return (
        <div className="max-w-[1080px] mx-auto pb-12">
            ...
            <div className="mb-8 flex items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Đăng ký đề tài mới</h1>
                    <p className="text-gray-500 mt-1">Giảng viên đăng ký và tự hướng dẫn đề tài.</p>
                </div>
            ...
            {/* Mode Selection */}
            <Tabs value={mode} onValueChange={(v) => setMode(v as ProposeMode)} className="mb-8">
                <TabsList className="bg-gray-100/50 p-1 border border-gray-200/50 w-full md:w-auto h-auto">
                    <TabsTrigger
                        ...
                        onClick={() => {
                            ...
                            setForm(prev => ({
                                ...prev,
                                title: "", description: "", objective: "", content: "", budget: "", note: ""
                            }));
                            setSelectedTopicId("");
                        }}
                    >
                                ...
                                <SelectContent>
                                    {rejectedTopics.map(topic => (
                                        <SelectItem key={topic.id} value={topic.id.toString()}>
                                            <span className="font-medium">{topic.code ? `[${topic.code}] ` : ""}</span>
                                            {topic.title}
                                            <span className="ml-2 text-xs text-gray-400">({new Date(topic.submittedAt).toLocaleDateString('vi-VN')})</span>
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
