...
/**
 * Trang Quản lý Phiên năm học (dành cho Trợ lý).
 * Cho phép tạo và quản lý các phiên đăng ký đề tài cho từng năm học của Khoa.
 * Quản lý trạng thái phiên: Đang đăng ký, Đang xét duyệt, Đang thực hiện, Hoàn thành.
 */
export default function YearSessionsPage() {
  const { user } = useAuth();
  const [data, setData] = useState<YearSession[]>([]);
  const [open, setOpen] = useState(false);
  const [editingSession, setEditingSession] = useState<YearSession | null>(
    null,
  );
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const message =
          ...
          "Không thể tải danh sách phiên năm học";
        toast.error(message);
      }
    };
    fetchData();
  ...
  return (
    <div className="h-full flex-1 flex-col space-y-8 md:flex">
      <div className="flex items-center justify-between space-y-2">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">
            Quản lý phiên năm học
          </h2>
          <p className="text-muted-foreground">
            Danh sách các phiên năm học của khoa.
          </p>
        </div>
      ...
      <YearSessionList sessions={data} onEdit={handleEdit} />
