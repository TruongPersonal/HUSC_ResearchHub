    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (selectedYear && currentUser?.departmentId) {
                const res = await topicService.getAll(debouncedSearch, status, currentUser.departmentId, selectedYear.id, currentPage, 10);
                setTopics(res.content);
                setTotalPages(res.totalPages);

                // Check Session Status
                const sessions = await yearSessionService.getAll(undefined, 0, 100, currentUser.departmentId);
                const currentSession = sessions.content.find(s => s.academicYearId === selectedYear.id);
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight text-gray-900">Quản lý hồ sơ</h1>
                    <p className="text-muted-foreground">
                        Danh sách các hồ sơ Khoa.
                    </p>
                </div>
            </div>
