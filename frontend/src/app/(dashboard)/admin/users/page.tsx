 ...
 * Hiển thị danh sách người dùng, hỗ trợ tìm kiếm, lọc theo vai trò/khoa.
 * Hỗ trợ Import Excel, Reset mật khẩu, và chỉnh sửa thông tin người dùng.
 */
export default function UsersPage() {
  ...
  useEffect(() => {
    const deptId =
      departmentFilter !== "0" ? parseInt(departmentFilter) : undefined;
    ...
    try {
      if (editingUser) {
        ...
        if (newUser.role === "STUDENT" || newUser.role === "TEACHER") {
          toast.success(
            "Tạo người dùng thành công. Mật khẩu đã được gửi về email @husc.edu.vn của người dùng.",
          );
        } else {
        ...
        filters={[
          {
            ...
            onValueChange: (val) =>
              setRoleFilter(val === "ALL" ? undefined : (val as UserRole)),
            options: [
