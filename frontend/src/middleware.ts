...
export function middleware(request: NextRequest) {
    ...
    const publicPaths = ['/login', '/images', '/icons']
    ...
    if (token) {
        try {
            ...
            if (request.nextUrl.pathname.startsWith('/student') && role !== 'ROLE_STUDENT') {
                return NextResponse.redirect(new URL('/', request.url))
            }
            if (request.nextUrl.pathname.startsWith('/teacher') && role !== 'ROLE_TEACHER') {
                return NextResponse.redirect(new URL('/', request.url))
            }
            if (request.nextUrl.pathname.startsWith('/assistant') && role !== 'ROLE_ASSISTANT') {
                return NextResponse.redirect(new URL('/', request.url))
            }
        } catch {
            return NextResponse.redirect(new URL('/login', request.url))
