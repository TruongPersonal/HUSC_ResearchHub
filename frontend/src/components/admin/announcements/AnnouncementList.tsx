...
export function AnnouncementList({ data, onEdit, onDelete }: AnnouncementListProps) {
    if (data.length === 0) {
        return (
    ...
    return (
        <div className="grid grid-cols-1 gap-4">
            {data.map((item) => (
                <Card key={item.id} className="group hover:shadow-md transition-all border-gray-200">
                    <CardHeader className="pb-2">
                        ...
                        <div className="flex items-center text-xs text-muted-foreground gap-1 mt-1">
                            ...
                            {item.publishDatetime
                                ? format(new Date(item.publishDatetime), "dd 'tháng' MM, yyyy 'lúc' HH:mm", { locale: vi })
                                : "Chưa xuất bản"}
                        </div>
                    </CardHeader>
                    <CardContent>
