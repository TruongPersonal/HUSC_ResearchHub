...
export function FacultyForm({ open, onOpenChange, initialData, onSubmit }: FacultyFormProps) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    <DialogTitle>{initialData ? "Chỉnh sửa khoa" : "Thêm khoa mới"}</DialogTitle>
                    <DialogDescription>
                        {initialData ? "Cập nhật thông tin khoa." : "Tạo khoa mới cho hệ thống."}
                    </DialogDescription>
                </DialogHeader>
                <Form {...form}>
