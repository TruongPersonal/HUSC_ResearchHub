...
function MobileMenuContent({ pathname, onClose }: { pathname: string; onClose: () => void }) {
  ...
  const handleLogout = () => {
    ...
    router.push("/login"); // router.push logic
    onClose(); // Close menu on logout too (optional but good UX)
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-6">
        ...
        <div className="space-y-1">
          ...
          <Link href="/student/profile" onClick={onClose} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200", pathname === "/student/profile" ? "bg-blue-600/10 text-blue-700" : "text-gray-600 hover:bg-gray-50")}>
            <User className={cn("w-5 h-5", pathname === "/student/profile" ? "text-blue-600" : "text-gray-400")} /> Thông tin cá nhân
          </Link>
          <Link href="/student/messages" onClick={onClose} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200", pathname === "/student/messages" ? "bg-blue-600/10 text-blue-700" : "text-gray-600 hover:bg-gray-50")}>
            <MessageSquare className={cn("w-5 h-5", pathname === "/student/messages" ? "text-blue-600" : "text-gray-400")} /> Tin nhắn
          </Link>
          <Link href="/student/announcements" onClick={onClose} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200", pathname === "/student/announcements" ? "bg-blue-600/10 text-blue-700" : "text-gray-600 hover:bg-gray-50")}>
            <Bell className={cn("w-5 h-5", pathname === "/student/announcements" ? "text-blue-600" : "text-gray-400")} /> Thông báo
          </Link>
          <Link href="/student/topic-propose" onClick={onClose} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200", pathname === "/student/topic-propose" ? "bg-blue-600/10 text-blue-700" : "text-gray-600 hover:bg-gray-50")}>
            <FilePlus className={cn("w-5 h-5", pathname === "/student/topic-propose" ? "text-blue-600" : "text-gray-400")} /> Đề xuất đề tài
          </Link>
        </div>
      </div>
