...
const studentItems = [
    ...
    },
    {
        ...
        icon: FileText, // Using FileText as a placeholder for Message icon if not imported
    },
    {
    ...
    return (
        <div className="flex h-full w-64 flex-col border-r bg-white">
            <div className="flex h-16 items-center border-b px-6 gap-3">
                <Image src="/images/icons/logo.png" alt="Logo" width={60} height={60} className="object-contain" />
                <span className="text-md font-bold tracking-tight text-blue-900">HUSC ResearchHub</span>
            </div>
            <div className="flex-1 overflow-y-auto py-4">
                <nav className="grid gap-1 px-2">
                    {items.map((item, index) => {
                        const Icon = item.icon
                        const isActive = item.href === "/admin" || item.href === "/student" || item.href === "/teacher" || item.href === "/assistant"
                            ? pathname === item.href
