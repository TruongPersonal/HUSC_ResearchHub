...
import { Settings2, Search, Calendar, User, GraduationCap, Clock, Users } from "lucide-react";
...
export function TopicRegistrationList({ topics, onOpenDetail, onOpenAction, onOpenMemberApproval, canApprove = true }: TopicRegistrationListProps) {
    if (topics.length === 0) {
        return (
                            ...
                            <div className="flex flex-col gap-2 text-sm text-gray-600">
                                ...
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pt-1">
                                    <div className="flex flex-wrap items-center gap-x-8 gap-y-2 text-muted-foreground">
                                        <div className="flex items-center gap-2 min-w-[200px]" title="Ngày đăng ký">
                                            <Calendar className="h-4 w-4 text-gray-400" />
                                            <span>{new Date(t.submittedAt).toLocaleDateString("vi-VN")}</span>
                                        </div>
                                    </div>
                                    ...
                                    {t.status !== "APPROVED" && t.status !== "REJECTED" && (
                                        <div className="flex justify-end gap-2 pt-2" onClick={(e) => e.stopPropagation()}>
                                            <Button
                                                ...
                                                onClick={() => onOpenMemberApproval ? onOpenMemberApproval(t) : onOpenAction(t)}
                                            >
