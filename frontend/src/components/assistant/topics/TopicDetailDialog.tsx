...
export function TopicDetailDialog({ open, onOpenChange, topic, onSave }: TopicDetailDialogProps) {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-start justify-between gap-3 mb-4">
                    <div className="text-lg font-semibold text-[#0b57a8]">{topic.title}</div>
                    {!editMode && topic.status !== "REJECTED" && topic.status !== "CANCELLED" && topic.status !== "CANCELED" && topic.approvedStatus !== "CANCELED" && topic.approvedStatus !== "CANCELLED" && topic.approvedStatus !== "COMPLETED" && topic.sessionStatus !== "COMPLETED" && (
                        <Button
                            size="sm"
                            ...
                            <>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                                    ...
                                    <div className="md:col-span-3 space-y-1">
                                        {topic.approvedMembers && topic.approvedMembers.length > 0 ? (
                                            topic.approvedMembers.map((student) => (
                                                <div key={student.id} className="text-gray-900">
                                                    {student.name}
                                                    {student.id === topic.studentLeaderId && (
                                                        <span className="text-blue-600 font-medium ml-2">(Chủ nhiệm)</span>
                                                    )}
                                                </div>
                                            ))
                                        ) : (
                                            <div className="text-gray-500 italic">Chưa có sinh viên thực hiện</div>
                                        )}
                                    </div>
                                </div>
                                    ...
                                    <div className="md:col-span-3 space-y-1">
                                        {topic.advisors && topic.advisors.length > 0 ? (
                                            ...
                                            topic.advisor ? (
                                                ...
                                                <div className="text-gray-500 italic">Chưa có giảng viên hướng dẫn</div>
                                            )
                                        )}
                                    </div>
                                </div>
