...
import { ApprovedTopicResponse, ApprovedTopicDocumentResponse, ApprovedTopicStatus } from "@/features/topics/types";
...
export function ApprovedTopicDetailDialog({ open, onOpenChange, topic, onSave }: ApprovedTopicDetailDialogProps) {
    // Editable state
    ...
    useEffect(() => {
        if (open && topic) {
            ...
            if (topic.status === ApprovedTopicStatus.COMPLETED || topic.status === ApprovedTopicStatus.CANCELED) {
            ...
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-6">
                    <div className="flex items-start justify-between gap-3">
                        ...
                        {!editMode && topic.status !== ApprovedTopicStatus.CANCELED && topic.topic.sessionStatus !== "COMPLETED" && (
                            <Button
                                size="sm"
                    ...
                    {!editMode ? (
                        <div className="space-y-6">
                            <div className="space-y-4">
                                ...
                                {(topic.status === ApprovedTopicStatus.CANCELED || topic.status === ApprovedTopicStatus.COMPLETED) && (
                                    <>
                                        <DetailRow label="Lĩnh vực">{topic.fieldResearch || "—"}</DetailRow>
                                        <DetailRow label="Loại hình">{topic.typeResearch || "—"}</DetailRow>
                                    </>
                                )}
                            </div>
                    ...
                    {(topic.status === ApprovedTopicStatus.COMPLETED || topic.status === ApprovedTopicStatus.CANCELED) && !editMode && (
                        <div className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                                ...
                                <div className="md:col-span-3 flex flex-col justify-center">
                                    {loadingDocs ? (
                                        ...
                                        <div className="text-sm text-gray-500 italic">Chưa có tài liệu báo cáo nào.</div>
                                    ) : (
                                        <div className="grid grid-cols-1 gap-2">
                                            {documents.map((doc) => {
                                                let typeLabel: string = doc.documentType;
                                                switch (doc.documentType) {
                                                    case "MIDTERM_REPORT": typeLabel = "Báo cáo giữa kỳ"; break;
                                                    case "SUMMARY_REPORT": typeLabel = "Báo cáo tổng kết"; break;
                                                    case "SCIENTIFIC_ARTICLE": typeLabel = "Bài báo khoa học"; break;
                                                    case "SCIENTIFIC_PRESENTATION": typeLabel = "Thuyết trình khoa học"; break;
                                                }
                                                    ...
                                                    >
                                                        <div className="flex items-center justify-between">
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={() => window.open(`http://localhost:8080${doc.fileUrl}`, "_blank")}
                                                            >
...
function DetailRow({ label, children }: { label: string; children: React.ReactNode }) {
    return (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
            ...
            <div className="md:col-span-3 text-gray-900 whitespace-pre-wrap">{children}</div>
        </div>
    );
}
