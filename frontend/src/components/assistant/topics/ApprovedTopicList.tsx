...
import {
  ApprovedTopicStatusBadge,
  getApprovedTopicStatusConfig,
} from "./ApprovedTopicStatusBadge";
...
/**
 * Danh sách Đề tài đã duyệt (trạng thái: Đang thực hiện, Hoàn thành, ...)
 * Hiển thị dạng thẻ (Card) với thông tin tóm tắt và các nút hành động (Xem báo cáo, Thao tác).
 */
export function ApprovedTopicList({
  topics,
  onOpenDetail,
  onOpenAction,
  onOpenReport,
}: ApprovedTopicListProps) {
  if (topics.length === 0) {
    return (
            ...
            <div className="px-4 flex flex-col gap-3">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  {t.code && (
                    <Badge
                      variant="outline"
                      className="bg-slate-50 text-slate-700 border-slate-200 py-0.5 px-2 mb-4 w-fit"
                    >
                      {t.code}
                    </Badge>
                  )}
              ...
              <div className="flex flex-col gap-2 text-sm text-gray-600">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-1">
                      {t.topic.approvedMembers &&
                        t.topic.approvedMembers.length > 0 ? (
                        t.topic.approvedMembers.map((s, idx) => (
                          <span key={s.id} className="text-gray-900">
                            {s.name}
                            {s.id === t.topic.studentLeaderId && (
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pt-1 mt-1">
                  ...
                  <div
                    className="flex justify-end gap-2 pt-2 md:pt-0"
