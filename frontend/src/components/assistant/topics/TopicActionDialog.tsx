...
export function TopicActionDialog({ open, onOpenChange, topic, onSuccess, departmentId, canApprove = true }: TopicActionDialogProps) {
    ...
    const [eligibleSupervisors, setEligibleSupervisors] = useState<{ id: number; fullName: string }[]>([]);
    ...
    useEffect(() => {
        if (open && topic) {
            ...
            const fetchSupervisors = async () => {
                ...
                if (selectedYear) {
                    try {
                        const users = await userService.getEligibleSupervisors(departmentId, selectedYear.id);
                        const supervisors = users.map(u => ({ id: u.id, fullName: u.fullName }));
                        ...
                        if (topic.advisorId && topic.advisor) {
                            ...
                            if (!exists) {
                                supervisors.push({ id: topic.advisorId, fullName: topic.advisor });
                            }
                        }
    ...
    const handleSave = async () => {
        try {
            ...
            if (formData.status === "NEEDS_UPDATE" && topic.status !== "NEEDS_UPDATE" && !formData.feedback?.trim()) {
            ...
            if (formData.status !== topic.status) {
                await topicService.updateStatus(topic.id, formData.status, formData.feedback);
            }
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                <div className="mb-4 text-lg font-semibold text-[#0b57a8]">{topic.title}</div>
                        ...
                        <div className="space-y-4">
                            <Field label="Chọn giảng viên hướng dẫn">
                                <Select
                                    ...
                                    onValueChange={(v) => setFormData({ ...formData, advisorId: parseInt(v) })}
                                    disabled={isLocked}
                                >
                            ...
                            <Field label="Chọn sinh viên chủ nhiệm">
                                <Select
                                    ...
                                    onValueChange={(v) => setFormData({ ...formData, studentLeaderId: parseInt(v) })}
                                    disabled={isLocked}
                                >
                                    <SelectTrigger className="w-full">
                            ...
                            <Field label="Trạng thái">
                                <Select
                                    ...
                                    onValueChange={(v) => setFormData({ ...formData, status: v as TopicStatus })}
                                    disabled={isLocked}
                                >
                                    <SelectTrigger className="w-full">
                                        <SelectValue />
                            ...
                            {formData.status === "NEEDS_UPDATE" && topic.status !== "NEEDS_UPDATE" && (
                                <Field label="Lý do">
                                    <Textarea
                ...
                <DialogFooter className="gap-2 sm:gap-0">
                    ...
                    {showRejectReason ? (
                        <div className="flex gap-2 w-full justify-end">
                            <Button variant="outline" onClick={() => setShowRejectReason(false)}>Hủy bỏ</Button>
                            <Button variant="destructive" onClick={handleRejectTopic}>Xác nhận từ chối</Button>
                        </div>
                    ) : (
                        <Button onClick={handleSave} disabled={isLocked}>Lưu thay đổi</Button>
                    )}
                </DialogFooter>
            </DialogContent>
        </Dialog>
