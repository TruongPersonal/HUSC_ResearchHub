...
/**
 * Form tạo/chỉnh sửa Phiên làm việc (Session) của Năm học cho Khoa.
 * Hỗ trợ chọn Năm học (khi tạo mới) và cập nhật Trạng thái (Đang đăng ký, Đang xét duyệt...).
 */
export function YearSessionForm({
  open,
  onOpenChange,
  initialData,
  onSubmit,
}: YearSessionFormProps) {
  ...
  const handleSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    await onSubmit({
      academicYearId: values.academicYearId
        ? parseInt(values.academicYearId)
        : undefined,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>
            {initialData ? "Chỉnh sửa phiên năm học" : "Thêm phiên năm học mới"}
          </DialogTitle>
          <DialogDescription>
            {initialData
              ? "Cập nhật thông tin phiên năm học."
              : "Tạo phiên năm học cho khoa."}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
          ...
          >
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    Trạng thái {initialData ? `- ${initialData.year}` : ""}
                  </FormLabel>
                  <Select
                    onValueChange={field.onChange}
                    defaultValue={field.value}
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="ON_REGISTRATION">
                        Đang đăng ký
                      </SelectItem>
            ...
            <DialogFooter>
              <Button
                type="submit"
                disabled={
                  loading || (availableYears.length === 0 && !initialData)
                }
              >
                Lưu thay đổi
              </Button>
